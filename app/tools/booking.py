...
def process_incoming_message(phone_number: str, customer_name: str, message: str, tenant, db: Session) -> str:
    ...
    print(f"[Tenant: {tenant.name}] Extracted -> Intent: {intent}, Date: {extracted_date}, Time: {extracted_time}, Preference: {extracted_time_preference}, Name: {extracted_name}")
    ...
    # 3. Contextual override: if we are expecting a time slot selection and the user
    # responds with something containing a time, force the *pending* intent.
    # Using pending_action (instead of hardcoding "book_appointment") prevents a
    # reschedule-in-progress from being silently converted into a new booking.
    ...
    if intent == "greeting":
        ...
        try:
            ...
            slots = filter_slots_by_preference(all_slots, extracted_time_preference)
            ...
            if not all_slots:
                ...
                # Slots exist that day but none in the requested time band — offer alternatives
                fallback_slots = filter_slots_by_preference(all_slots, None)
                slots_str = "\n".join([f"- {s}" for s in fallback_slots])
                reply_context = f"no_{extracted_time_preference}_slots for {target_date}, proposing alternatives:\n{slots_str}"
        ...
        if not target_date:
            ...
            try:
                ...
                if not all_slots:
                    ...
                    # Slots exist but none in the requested band — offer alternatives
                    fallback_slots = filter_slots_by_preference(all_slots, None)
                    slots_str = "\n".join([f"- {s}" for s in fallback_slots])
                    reply_text = (
                        f"Non ho disponibilita' nel {extracted_time_preference or 'orario richiesto'} per il {target_date}.\n"
